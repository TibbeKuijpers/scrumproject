...
<title>Untitled Document</title>
	...
	<link rel="stylesheet" type="text/css" href="profielpagina.css">
</head>
...
<body>
  ...
  <div class="form-group row">
    ...
    <div class="col-sm-10">
      <input type="text" readonly  id="staticEmail" value="Wachtwoord">
    </div>
  </div>
</form>
	</div>
	<hr>
		<h1 align="center">Onlangs besteld</h1>
		<div id="content">
		<div id="filter">
			<br><br>
			...
			<div id="card1" style="width: 18rem;">
  				...
  				<div class="card-body">
					<h5 class="card-title"><?php $sql='SELECT `fietsNaam`FROM `fietsen` WHERE fietsID=3';
												 $result = $connect->query($sql);
												 if ($result->num_rows > 0) {
												 while($row = $result->fetch_assoc()) {
												 echo $row['fietsNaam'];
    }}											 ?></h5>
					<p class="card-text"><?php $sql='SELECT `fietsMerk`, `prijsFiets` FROM `fietsen` WHERE fietsID=3';
												 $result = $connect->query($sql);
												 if ($result->num_rows > 0) {
												 while($row = $result->fetch_assoc()) {
			...
			<div id="card2" style="width: 18rem;">
  				<img src="Images/fiets_heren.jpg" class="card-img-top" alt="...">
  				<div class="card-body">
    				<h5 class="card-title"><?php $sql='SELECT `fietsNaam`FROM `fietsen` WHERE fietsID=2';
												 $result = $connect->query($sql);
												 if ($result->num_rows > 0) {
												 while($row = $result->fetch_assoc()) {
												 echo $row['fietsNaam'];
    }}											 ?></h5>
   					<p class="card-text"><?php $sql='SELECT `fietsMerk`, `prijsFiets` FROM `fietsen` WHERE fietsID=2';
												 $result = $connect->query($sql);
												 if ($result->num_rows > 0) {
												 while($row = $result->fetch_assoc()) {
												 echo $row['fietsMerk'] . "<br> €" . $row['prijsFiets'] . ",-";
